...
class _LiveScreenState extends State<LiveScreen> {
  ...
  void _togglePlay() {
    ...
    if (_playing) {
      _timer = Timer.periodic(const Duration(milliseconds: 100), (_) {
        setState(() { _time += 0.1; if (_time >= _total) { _time = 0; _playing = false; _timer?.cancel(); } });
      });
    } else { _timer?.cancel(); }
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('LIVE'), actions: [
        if (widget.stemJobId != null) IconButton(
          ...
          onPressed: () { SystemChrome.setEnabledSystemUIMode(SystemUiMode.immersiveSticky); setState(() => _stageMode = true); }),
      ]),
          ...
          : SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(children: [
                ...
                Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  ...
                  GestureDetector(onTap: _togglePlay,
                    child: Container(width: 60, height: 60, decoration: BoxDecoration(
                      shape: BoxShape.circle, color: _playing ? BksColors.gold : const Color(0xFF2A2000),
                      border: Border.all(color: BksColors.gold, width: 2)),
                      child: Icon(_playing ? Icons.pause_rounded : Icons.play_arrow_rounded,
                          color: _playing ? BksColors.textInverse : BksColors.gold, size: 32))),
                  IconButton(onPressed: () => setState(() => _time = _total),
                      icon: const Icon(Icons.skip_next_rounded, color: BksColors.textSecondary, size: 30)),
                ]),
              ])),
              ...
              _card(Column(children: [
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  const Text('TRANSPOSE', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700,
                      color: BksColors.textMuted, letterSpacing: 2)),
                  Text(_transpose == 0 ? '0 ST' : '${_transpose > 0 ? '+' : ''}${_transpose.toInt()} ST',
                ...
                Row(children: [-4,-3,-2,-1,0,1,2,3,4].map((s) => Expanded(child: Padding(
                  ...
                  child: GestureDetector(onTap: () => setState(() => _transpose = s.toDouble()),
                    child: Container(height: 38, decoration: BoxDecoration(
                      ...
                      border: Border.all(color: _transpose == s ? BksColors.gold : BksColors.border)),
              ...
              _card(Row(children: [
                ...
                Expanded(child: SliderTheme(
                  data: const SliderThemeData(activeTrackColor: BksColors.bass,
                      inactiveTrackColor: BksColors.bg3, thumbColor: BksColors.bass, trackHeight: 3),
                  child: Slider(value: _tempo, min: 70, max: 130,
                      onChanged: (v) => setState(() => _tempo = v)))),
                Text('${_tempo.round()}%', style: const TextStyle(fontSize: 15,
                    fontWeight: FontWeight.w700, color: BksColors.bass)),
              ])),
              const SizedBox(height: 12),
              // Lyrics preview
              if (_lyrics.isNotEmpty) Container(
                padding: const EdgeInsets.all(18),
                decoration: BoxDecoration(color: const Color(0xFF0A0800),
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: BksColors.goldDim)),
                child: Column(children: [
                  Text(_activeIdx >= 0 ? _lyrics[_activeIdx].text : '♪',
                      style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700,
                          color: BksColors.textPrimary), textAlign: TextAlign.center),
              ...
              SizedBox(width: double.infinity, child: ElevatedButton.icon(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: const Color(0xFF2A1A00), foregroundColor: BksColors.gold,
  ...
  Widget _buildStage() {
    ...
    final next = _activeIdx >= 0 && _activeIdx + 1 < _lyrics.length ? _lyrics[_activeIdx + 1] : null;
    return Scaffold(backgroundColor: Colors.black, body: SafeArea(child: Column(children: [
      Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
        ...
        child: Row(children: [
          ...
          const Spacer(),
          Text('${_tempo.round()}% BPM', style: const TextStyle(
              color: Colors.white38, fontSize: 12, letterSpacing: 2)),
          const SizedBox(width: 16),
          Text(_transpose == 0 ? '– ST' : '${_transpose > 0 ? '+' : ''}${_transpose.toInt()} ST',
              style: TextStyle(color: _transpose == 0 ? Colors.white38 : BksColors.gold,
                  fontSize: 12, fontWeight: FontWeight.w700, letterSpacing: 2)),
        ])),
      Expanded(child: GestureDetector(onTap: _togglePlay,
        child: Container(color: Colors.black, alignment: Alignment.center,
          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            if (next != null) ...[const SizedBox(height: 16),
              Text(next.text, style: const TextStyle(fontSize: 20, color: Color(0xFF3A3020)),
                  textAlign: TextAlign.center)],
            if (active == null) Icon(_playing ? Icons.pause_circle : Icons.play_circle,
                color: Colors.white12, size: 80),
          ])))),
      Container(color: const Color(0xFF080600), padding: const EdgeInsets.all(14), child: Column(children: [
        LinearProgressIndicator(value: _time / _total,
            backgroundColor: Colors.white10, color: BksColors.gold, minHeight: 3),
        const SizedBox(height: 12),
        Row(children: [
          _stageBtn('−ST', BksColors.vocals, () => setState(() => _transpose = (_transpose - 1).clamp(-12, 12))),
          const SizedBox(width: 8),
          Expanded(flex: 2, child: _stageBtnLarge(_playing ? '⏸' : '▶', BksColors.gold, _togglePlay)),
          const SizedBox(width: 8),
          _stageBtn('+ST', BksColors.instr, () => setState(() => _transpose = (_transpose + 1).clamp(-12, 12))),
        ]),
        const SizedBox(height: 8),
        Row(children: [
          _stageBtn('−BPM', BksColors.bass, () => setState(() => _tempo = (_tempo - 5).clamp(70, 130))),
          const SizedBox(width: 8),
          _stageBtn('RESET', BksColors.textMuted, () => setState(() { _transpose = 0; _tempo = 100; })),
          const SizedBox(width: 8),
          _stageBtn('+BPM', BksColors.bass, () => setState(() => _tempo = (_tempo + 5).clamp(70, 130))),
        ]),
      ])),
  ...
  Widget _card(Widget child) => Container(margin: const EdgeInsets.only(bottom: 0),
    padding: const EdgeInsets.all(14),
    decoration: BoxDecoration(color: BksColors.bg1, borderRadius: BorderRadius.circular(12),
  ...
  Widget _stageBtn(String label, Color color, VoidCallback onTap) => Expanded(
    child: GestureDetector(onTap: onTap,
      child: Container(height: 50, decoration: BoxDecoration(
        ...
        alignment: Alignment.center,
        child: Text(label, style: TextStyle(color: color, fontSize: 16, fontWeight: FontWeight.w700)))));

  Widget _stageBtnLarge(String label, Color color, VoidCallback onTap) => GestureDetector(onTap: onTap,
    child: Container(height: 70, decoration: BoxDecoration(
      ...
      alignment: Alignment.center,
      child: Text(label, style: TextStyle(color: color, fontSize: 28, fontWeight: FontWeight.w700))));
}
